"""
Fixed Advanced Visualizer Component with Real-Time Data Updates
Includes the missing update_with_real_time_data method and maintains all existing functionality.
"""
...
class AdvancedVisualizerWithMomentum:
    """Advanced visualizer with momentum vectors, time stepping, and real-time updates."""
    ...
    def setup_enhanced_text_display(self):
        ...
        header = """
╔══════════════════════════════════════════════════════════════════════════════════════════════╗
║ 🚀 ULTIMATE NUCLEAR PHYSICS SIMULATOR WITH ADVANCED FORCES ║
║ Modern QCD-Based Nuclear Forces + All Enhanced Features ║
╚══════════════════════════════════════════════════════════════════════════════════════════════╝
...
🔬 ADVANCED NUCLEAR FORCE FEATURES:
• Chiral Effective Field Theory (χEFT) - Modern QCD-based forces
• Argonne v18 potential - High-precision phenomenological model  
• QCD sum rules - Direct connection to quark/gluon physics
• Lattice QCD inspired - First-principles calculations
• Enhanced multi-meson Yukawa - Improved fallback model
• Real-time force model switching
...
"""
    ...
    def update_with_real_time_data(self, simulation_engine):
        """
        NEW METHOD: Update visualization with real-time data from simulation engine.
        
        This method is called during simulation to provide real-time updates.
        """
        ...
        try:
            ...
            if current_time - self.last_update_time < 0.1:  # Update at most every 100ms
                return
            
            self.last_update_time = current_time
            
            # Create simulation data structure from engine state
            current_particles = getattr(simulation_engine, 'particles', [])
            current_time_sim = getattr(simulation_engine, 'current_time', 0.0)
            
            # Get global observables
            global_obs = getattr(simulation_engine, 'global_observables', {})
            
            # Get boundary conditions info
            boundary_conditions = getattr(simulation_engine, 'boundary_conditions', None)
            boundary_info = {}
            if boundary_conditions:
                boundary_info = boundary_conditions.get_status_info()
            
            # Get nuclear force info
            nuclear_force_manager = getattr(simulation_engine, 'nuclear_force_manager', None)
            force_model = 'Unknown'
            if nuclear_force_manager:
                force_model = nuclear_force_manager.get_current_model_name()
            
            # Create real-time state
            real_time_state = {
                'time': current_time_sim,
                'particles': [p.copy() for p in current_particles] if current_particles else [],
                'boundary_status': boundary_info,
                'collision_started': boundary_info.get('collision_started', False),
                'escaped_mass_fraction': boundary_info.get('escaped_mass_fraction', 0.0),
                'total_reactions': len(getattr(simulation_engine.equation_tracker, 'reactions', [])) if hasattr(simulation_engine, 'equation_tracker') else 0,
                'nuclear_force_model': force_model
            }
            
            # Create simplified simulation data for real-time update
            real_time_sim_data = {
                'global_observables': global_obs,
                'time_history': [real_time_state],  # Single current state
                'real_time_mode': True
            }
            
            # Update visualization
            if MATPLOTLIB_AVAILABLE and hasattr(self, 'fig'):
                self._update_real_time_matplotlib(real_time_sim_data)
            else:
                self._update_real_time_text(real_time_sim_data)
                
        except Exception as e:
            print(f"Real-time visualization update error: {e}")
            # Don't let visualization errors crash the simulation
            pass
    
    def _update_real_time_matplotlib(self, real_time_data: Dict):
        """Update matplotlib plots with real-time data."""
        
        try:
            # Update 3D plot with current particles
            self.ax_3d.clear()
            self.ax_3d.set_facecolor('#2e2e2e')
            
            current_state = real_time_data['time_history'][0]
            current_time = current_state['time']
            
            # Enhanced 3D plot title with current time and force model
            force_model = current_state.get('nuclear_force_model', 'Unknown')
            self.ax_3d.set_title(
                f'Real-Time Nuclear Collision\\n'
                f'Time: {current_time:.3f} fm/c | Forces: {force_model}', 
            ...
            self._update_physics_plots_real_time(real_time_data)
            ...
            print(f"Real-time matplotlib update error: {e}")
    
    def _update_real_time_text(self, real_time_data: Dict):
        """Update text display with real-time data."""
        ...
        try:
            ...
            status = f"""
╔══════════════════════════════════════════════════════════════════════════════════════════════╗
║ 🚀 REAL-TIME NUCLEAR COLLISION ANALYSIS ║
...
│ Active Particles: {len(particles):5d} │
│ Nuclear Force Model: {current_state.get('nuclear_force_model', 'Unknown'):<25s} │
│ Collision Started: {'✅ YES' if current_state.get('collision_started', False) else '❌ NO'} │
│ Mass Escaped: {current_state.get('escaped_mass_fraction', 0):.1%} │
│ Total Reactions: {current_state.get('total_reactions', 0):5d} │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
"""
...
"""
            ...
            print(f"Real-time text update error: {e}")
    ...
    def _update_advanced_matplotlib(self):
        ...
        self.ax_3d.set_title(
            f'3D Nuclear Collision with Advanced Forces\\n'
    ...
    def _update_physics_plots_real_time(self, real_time_data: Dict):
        """Update physics plots with real-time data."""
        
        try:
            # For real-time updates, we might not have full time series
            # Just update with current values or show simplified plots
            
            global_obs = real_time_data.get('global_observables', {})
            
            # If we have time series data, use it
            if global_obs.get('time') and len(global_obs['time']) > 1:
                self._update_physics_plots_with_values()
            else:
                # For real-time mode, show current values only
                current_state = real_time_data['time_history'][0]
                current_time = current_state['time']
                
                # Show current time on all plots
                for ax in [self.ax_energy, self.ax_temp, self.ax_pressure, self.ax_entropy]:
                    ax.clear()
                    ax.set_facecolor('#2e2e2e')
                    ax.grid(True, alpha=0.3, color='white')
                    ax.tick_params(colors='white')
                    ax.text(0.5, 0.5, f'Real-time Mode\\nTime: {current_time:.3f} fm/c', 
                           transform=ax.transAxes, ha='center', va='center',
                           fontsize=12, color='white')
                    
        except Exception as e:
            print(f"Real-time physics plot update error: {e}")
    ...
    def _add_comprehensive_legend(self):
        ...
        try:
            ...
            if handles:
                ...
                legend.set_title("Nuclear Products & Advanced Forces", 
    ...
    def _update_enhanced_text(self):
        ...
        status = f"""
╔══════════════════════════════════════════════════════════════════════════════════════════════╗
║ 🚀 ADVANCED NUCLEAR COLLISION ANALYSIS WITH QCD FORCES ║
║ Time: {current_time:8.3f} fm/c ║
╚══════════════════════════════════════════════════════════════════════════════════════════════╝
...
│ Physics Engine: Advanced QCD-Based Nuclear Forces │
│ Nuclear Force Model: {current_state.get('nuclear_force_model', 'Unknown') if current_state else 'Unknown'} │
...
class LowEnergyStatusDisplay:
    ...
    def show_initial_status(self):
        ...
        initial_text = """
╔═══════════════════════════════════════════════════════════════════════════════════════════════════════╗
║ 🔬 ULTRA-LOW ENERGY NUCLEAR PHYSICS STATUS WITH ADVANCED FORCES ║
║ Advanced Shell Model Analysis + Modern QCD Nuclear Forces ║
╚═══════════════════════════════════════════════════════════════════════════════════════════════════════╝
...
│ • Chiral EFT: Modern QCD-based forces │
│ • Argonne v18: High-precision phenomenology │
│ • QCD Sum Rules: Direct quark-gluon connection │
│ • Enhanced Yukawa: Multi-meson exchange │
│ │
└─────────────────────────────────────────────────────────────────────────┘
...
"""
    ...
    def update_low_energy_status(self, simulation_data, time_index=-1):
        ...
        status = f"""
╔═══════════════════════════════════════════════════════════════════════════════════════════════════════╗
║ 🔬 LOW ENERGY NUCLEAR COLLISION ANALYSIS WITH ADVANCED FORCES ║
║ Time: {current_time:8.3f} fm/c ║
╚═══════════════════════════════════════════════════════════════════════════════════════════════════════╝
...
"""
        ...
        if obs.get('temperature'):
            ...
            # Nuclear force info
            force_model = 'Unknown'
            if current_state and 'nuclear_force_model' in current_state:
                force_model = current_state['nuclear_force_model']
...
│ Nuclear Force Model: {force_model} │
│ │
│ Shell Model Status: │
"""
    ...
    def _apply_text_highlighting(self):
        ...
        for i, line in enumerate(lines):
            ...
            # Physics terms
            elif any(term in line for term in ['Shell', 'Pairing', 'Magic', 'Transition', 'Force']):
